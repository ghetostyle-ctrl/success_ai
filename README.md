...
## 0. Claude Code 사용자 (가장 쉬운 방법)

Claude Code 를 쓰고 있다면 아래 프롬프트를 그대로 붙여넣으세요. Claude 가 이 README 를 읽고 설치·키 안내·실행·스케줄 등록까지 진행합니다.

```
https://github.com/ghetostyle-ctrl/success_ai 이 저장소를 내 PC에 설치해줘.
...
**Claude 에게 (에이전트 체크리스트)**
1. `node -v` 가 20 이상인지, `git` 이 있는지 확인. 없으면 설치 안내 후 중단.
...
> 다른 사람이 쓰려면 각자 자기 PC 에 설치해야 합니다.
...
저장한 문구·전사·출처를 로컬 제작 도구로 전달할 때는 [읽기 전용 레퍼런스 내보내기 API](REFERENCE_EXPORT.md)를 사용하세요.
메타 광고 화면의 **Meta Ad Studio로 보내기**를 누르면 로컬 Studio의 프로젝트 선택·가져오기 화면이 열립니다. 저장된 브랜드를 보고 있었다면 검색어가 채워집니다. Studio에서 광고를 미리 확인하고 프로젝트에 저장해야 전달이 완료됩니다. 두 앱이 같은 컴퓨터에서 실행 중이어야 합니다.
