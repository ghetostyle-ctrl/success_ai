...
구글 광고 소재 카드 또는 분석 표의 체크박스로 광고를 고른 뒤 **선택 링크 복사**를 누르면 `광고주 | URL` 목록(YouTube 영상은 영상 URL, 그 외 광고는 구글 광고 원본 URL)이 클립보드에 복사됩니다. **AD FACTORY로 보내기**는 같은 컴퓨터의 `http://127.0.0.1:4317`에서 프로젝트를 불러옵니다. 기존 프로젝트를 선택하거나 새 프로젝트를 만든 후 **저장**을 누르면 선택한 광고 ID만 위 내보내기 형식으로 전달합니다(한 번에 최대 100개). 분석 표에서 동일 영상으로 묶인 행을 고르면 그 행의 모든 광고 ID를 선택합니다. 같은 광고를 같은 프로젝트에 다시 보내면 원본 식별자로 갱신됩니다. 미리보기 주소와 이미 저장된 문구·전사를 함께 전달하며, 누락된 미디어를 새로 수집하지는 않습니다. 이 로컬 연결은 클라우드 Success AI에서 사용할 수 없습니다.
