    ...
    <section className="space-y-4" aria-label={`${advertiserName} 광고 소재`}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {adsWithPreviewsFirst.map((ad) => {
          ...
          return (
            <a key={ad.id} href={creativeUrl(ad)} target="_blank" rel="noopener noreferrer" className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-sm transition hover:border-[var(--accent)] hover:shadow-md">
